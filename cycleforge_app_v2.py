...
def role_points(row, role_name):
    r = ROLES[role_name]
    return pts_sb(int(row["sb_level"])) * r["sb"] + pts_mag(int(row["mag_level"])) * r["mag"]

def calc_feasible_quotas(df, energy_cap, recipe):
    sb_capacity = sum((energy_cap // 7) for _,row in df.iterrows() if int(row["sb_level"])>0)
    mag_capacity = sum((energy_cap // 2) for _,row in df.iterrows() if int(row["mag_level"])>0)
...
def assign_roles_sb_first(players_df, quotas, energy_cap):
    df = players_df.copy()
    ...
    # ---- Phase A: satisfy SB quota with SB-inclusive roles ----
    # build candidate list (player, role) for SB-inclusive roles
    sb_roles = ["SB-only","2 SB + 3 Mag","1 SB + 7 Mag"]
    candidates = []
    for idx, row in df.iterrows():
        for role_name in sb_roles:
            if not feasible_role(row, role_name, energy_cap):
                continue
            r = ROLES[role_name]
            if r["sb"] == 0:
                continue
            # value per SB cast
            total_val = role_points(row, role_name)
            v_per_sb = total_val / r["sb"]
            mag_per_sb = r["mag"] / r["sb"]
            candidates.append((v_per_sb, total_val, -mag_per_sb, idx, role_name, r))

    # sort: value per SB desc, then total value desc, then lower mag per SB
    ...
    for _, _, _, idx, role_name, r in candidates:
        ...
        if idx in assigned:
            continue
        # need to ensure quotas permit this pick
        if r["sb"] <= remaining_sb and r["mag"] <= remaining_mag:
            assigned[idx] = role_name
            remaining_sb  -= r["sb"]
            remaining_mag -= r["mag"]

    # ---- Phase B: spend remaining Mag quota with Mag-only ----
    mag_candidates = []
    for idx, row in df.iterrows():
        if idx in assigned:
            continue
        if feasible_role(row, "Mag-only", energy_cap):
            r = ROLES["Mag-only"]
            # value per Mag cast
            per_mag = df.loc[idx,"pts_per_mag"]
            total_val = per_mag * r["mag"]
            mag_candidates.append((per_mag, total_val, idx, r))
    # sort by per-mag value, then total value
    mag_candidates.sort(key=lambda t: (t[0], t[1]), reverse=True)
    for per_mag, total_val, idx, r in mag_candidates:
        if remaining_mag <= 0:
            break
        if r["mag"] <= remaining_mag:
...
if assign_btn:
    try:
        ...
        feasible = quotas
        plan_df, role_counts, totals, rem_sb, rem_mag = assign_roles_sb_first(players_df, quotas, energy_cap)

        # Update expected totals with actual
...
# ---------- Feasible Cycle (based on roster) ----------
